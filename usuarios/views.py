...
from .forms import EmpleadoForm, VacacionForm, AsistenciaForm, FiniquitoForm
from .models import Empleado, Asistencia, Vacacion, PrimaVacacional, Finiquito
from .utils import generar_primas_faltantes_para_empleado
...
def obtener_feriados(año):
    ...
    return Feriado.objects.filter(fecha__year=año).values_list('fecha', flat=True)
...
def calcular_dias_usados(empleado, año):
...
def dashboard(request):
    ...
    if empleado and empleado.es_administrador:
    ...
    # ⚠️ Preparado para futuros modelos de solicitudes
    ...
    # Suma total de todas las solicitudes pendientes
    total_solicitudes_pendientes = vacaciones_pendientes  # + permisos_pendientes + prestamos_pendientes


    return render(request, 'usuarios/dashboard.html', {
        'empleado': empleado, 'total_solicitudes_pendientes': total_solicitudes_pendientes
    })
...
def empleados_lista(request):
    if request.user.is_superuser:
        ...
        for empleado in empleados:
            generar_primas_faltantes_para_empleado(empleado)
...
def vacacion_pdf(request, vacacion_id):
    ...
    # Calcular días por ley y días usados correctamente
    año = vacacion.fecha_inicio.year
    dias_disponibles_totales = calcular_vacaciones(empleado.fecha_ingreso)
    dias_usados = calcular_dias_usados(empleado, año)
    feriados = obtener_feriados(año)

    # Calcular días de esta solicitud
    dias_esta_solicitud = 0
    dia = vacacion.fecha_inicio
    while dia <= vacacion.fecha_fin:
        if dia.weekday() != 6 and dia not in feriados:
            dias_esta_solicitud += 1
        dia += timedelta(days=1)

    dias_tomados_total = dias_usados  # ya incluye todas
    ...
    escribir_linea(f"Días de vacaciones correspondientes del período actual: {dias_disponibles_totales}")
    ...
    escribir_linea(f"Días tomados contando esta solicitud: {dias_usados}")
...
@login_required
def solicitudes_empleado(request, empleado_id):
    # 1. Obtener al empleado logueado
    empleado = get_object_or_404(Empleado, id=empleado_id)
    
    # 2. Filtrar sus solicitudes de vacaciones, ordenadas de la más reciente a la más antigua
    solicitudes = Vacacion.objects.filter(empleado=empleado).order_by('-fecha_inicio')

    # 3. Renderizar la plantilla solicitudes_empleado.html enviando el empleado y sus solicitudes
...
def login_admin_kiosko(request):
    if request.method == 'POST':
        ...
        if user is not None and user.is_staff:
            login(request, user)
            # En lugar de redirigir de inmediato, mostramos HTML con JS que guarda localStorage
            return HttpResponse("""
                <script>
                  localStorage.setItem("admin_kiosko_logueado", "1");
                  window.location.href = '/demo_admin/';
                </script>
            """)
        else:
            return HttpResponse("Credenciales inválidas o no eres administrador.")

@login_required
def demo_ausencia(request):
    if not request.user.empleado.es_administrador:
        return redirect('kiosko_asistencia')
    return HttpResponse("<h1>Demo: Ausencia de Registro</h1><p>Aquí irá la explicación del caso.</p>")

@login_required
def demo_continuos(request):
    if not request.user.empleado.es_administrador:
        return redirect('kiosko_asistencia')
    return HttpResponse("<h1>Demo: Registros Continuos</h1><p>Aquí irá la explicación del caso.</p>")

@login_required
def demo_corrido(request):
    if not request.user.empleado.es_administrador:
        return redirect('kiosko_asistencia')
    return HttpResponse("<h1>Demo: Horarios de Corrido</h1><p>Aquí irá la explicación del caso.</p>")

@login_required
def demo_admin(request):
    if not request.user.empleado.es_administrador:
        return redirect('login_admin_kiosko')
    return render(request, 'kiosko/demo_admin.html')


def demo_admin(request):
    return render(request, 'usuarios/demo_admin.html')

@login_required
def eventuales(request):
    return render(request, 'usuarios/eventuales.html')


@login_required
def eventual_nuevo(request):
    if request.method == 'POST':
        form = EventualForm(request.POST, request.FILES)
        if form.is_valid():
            form.save()
            messages.success(request, "Eventual registrado correctamente.")
            return redirect('eventuales')
    else:
        form = EventualForm()

    return render(request, 'usuarios/eventual_form.html', {
        'form': form
    })
...
def dashboard_admin(request):
    if request.user.is_superuser:
        # Si es superusuario, no tiene empleado asociado
        return render(request, 'usuarios/dashboard_admin.html', {'empleado': None})
    
    # Si es admin normal, buscar su empleado
    empleado = get_object_or_404(Empleado, user=request.user)
    return render(request, 'usuarios/dashboard_admin.html', {'empleado': empleado})


@login_required
def dashboard_usuario(request):
    empleado = get_object_or_404(Empleado, user=request.user)
    return render(request, 'usuarios/dashboard_usuario.html', {'empleado': empleado})


@login_required
def dashboard(request):
    # Si es superusuario, no requiere Empleado asociado
    if request.user.is_superuser:
        return render(request, 'usuarios/dashboard_admin.html', {'empleado': None})

    # Caso contrario, obtener su empleado
    empleado = get_object_or_404(Empleado, user=request.user)
...
@login_required
def primas_vacacionales(request):
    if request.user.is_superuser:
        empleados = Empleado.objects.filter(
            activo=True,
            primavacacional__pagado=False
        ).distinct()
    elif hasattr(request.user, 'empleado') and request.user.empleado.es_administrador:
        sucursal_admin = request.user.empleado.sucursal
        empleados = Empleado.objects.filter(
    ...
    return render(request, 'usuarios/primas_vacacionales.html', {'empleados': empleados})


@login_required
def primas_empleado(request, empleado_id):
    empleado = get_object_or_404(Empleado, id=empleado_id)

    if request.user.is_superuser or (
        hasattr(request.user, 'empleado') and request.user.empleado.es_administrador and request.user.empleado.sucursal == empleado.sucursal
    ):
        primas = PrimaVacacional.objects.filter(empleado=empleado).order_by('-anio')
    else:
        primas = PrimaVacacional.objects.none()

    return render(request, 'usuarios/primas_empleado.html', {
        'empleado': empleado,
        'primas': primas,
    })
...
@login_required
def marcar_prima_pagada(request, prima_id):
    prima = get_object_or_404(PrimaVacacional, id=prima_id)

    # Validar permisos: superusuario o administrador de la misma sucursal
    if not request.user.is_superuser:
        if not (hasattr(request.user, 'empleado') and request.user.empleado.es_administrador and request.user.empleado.sucursal == prima.empleado.sucursal):
            return HttpResponseRedirect(reverse('primas_vacacionales'))

    # Calcular fecha de pago según quincena
    hoy = date.today()
    if hoy.day <= 15:
        fecha_pago = hoy.replace(day=15)
    else:
        fecha_pago = hoy.replace(day=30 if hoy.month != 2 else 28)

    # Marcar como pagado
    prima.pagado = True
    prima.fecha_pago = fecha_pago
    prima.save()

    return HttpResponseRedirect(reverse('primas_empleado', args=[prima.empleado.id]))

@login_required
def documento_prima(request, prima_id):
    prima = get_object_or_404(PrimaVacacional, id=prima_id)

    # Calcular días de vacaciones según año cumplido
    dias_vacaciones = calcular_vacaciones(prima.anio)

    # Calcular sueldo diario y bruto
    try:
        sueldo_diario = float(prima.empleado.sueldo_quincenal) / 15
    except:
        sueldo_diario = 0

    sueldo_bruto = sueldo_diario * dias_vacaciones

    return render(request, 'usuarios/documento_prima.html', {
        'prima': prima,
        'dias_vacaciones': dias_vacaciones,
        'sueldo_diario': f"{sueldo_diario:.2f}",
        'sueldo_bruto': f"{sueldo_bruto:.2f}",
    })


def calcular_dias_vacaciones(anios_antiguedad):
    """
    Retorna el número de días de vacaciones que corresponden según el año de antigüedad.
    Esta tabla se basa en la Ley Federal del Trabajo (México) vigente.
    """
    if anios_antiguedad == 1:
        ...
        return 20
    elif 6 <= anios_antiguedad <= 10:
        return 22
    elif 11 <= anios_antiguedad <= 15:
        return 24
    elif 16 <= anios_antiguedad <= 20:
        return 26
    elif 21 <= anios_antiguedad <= 25:
        return 28
    elif 26 <= anios_antiguedad <= 30:
        return 30
    else:
        return 12  # Valor por defecto o mínimo


@login_required
def documento_prima_pdf(request, prima_id):
    prima = get_object_or_404(PrimaVacacional, id=prima_id)
    dias_vacaciones = calcular_dias_vacaciones(prima.anio)
    ...
    html_string = render_to_string('usuarios/documento_prima.html', {
        'prima': prima,
        'dias_vacaciones': dias_vacaciones,
        'sueldo_diario': f"{sueldo_diario:.2f}",
        'sueldo_bruto': f"{sueldo_bruto:.2f}",
    },
     request=request  # 👈 esto es lo que soluciona el error
    ...
    response['Content-Disposition'] = f'attachment; filename="prima_vacacional_{prima.empleado.numero_empleado}.pdf"'
    return response


@login_required
def finiquito_lista(request):
    # Solo empleados dados de baja (con fecha_baja y activo=False)
    empleados_baja = Empleado.objects.filter(activo=False).order_by('-fecha_baja')

    return render(request, 'usuarios/finiquito_lista.html', {
        'empleados_baja': empleados_baja
    })


@login_required
def registrar_finiquito(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            dias_trabajados = (fecha_baja - fecha_ingreso).days

            # Salario diario a partir de sueldo quincenal
            try:
                salario_diario = float(empleado.sueldo_quincenal) / 15
            except:
                salario_diario = 0

            # Aguinaldo proporcional
            if dias_trabajados >= 30:
                meses = dias_trabajados // 30
                aguinaldo_dias = 1.25 * meses
            else:
                aguinaldo_dias = (1.25 / 30) * dias_trabajados
            aguinaldo = salario_diario * aguinaldo_dias

            # Vacaciones proporcionales
            if dias_trabajados >= 30:
                vacaciones_dias = dias_trabajados // 30
            else:
                vacaciones_dias = (1 / 30) * dias_trabajados
            vacaciones = salario_diario * vacaciones_dias

            # Prima vacacional
            prima_vacacional = vacaciones * 0.25

            # Compensación (horas extra por valor definido)
            try:
                valor_hora_extra = float(empleado.horas_extras)
            except:
                valor_hora_extra = 0
            compensacion = finiquito.horas_extra * valor_hora_extra

            # Total bruto antes de adicionales y descuentos
            total = aguinaldo + vacaciones + prima_vacacional + compensacion

            # Agregamos lo adicional si existe
            if finiquito.monto_adicional:
                total += finiquito.monto_adicional
            if finiquito.descuento:
                total -= finiquito.descuento

            # Asignamos y guardamos
            finiquito.aguinaldo = round(aguinaldo, 2)
            finiquito.vacaciones = round(vacaciones, 2)
            finiquito.prima_vacacional = round(prima_vacacional, 2)
            finiquito.compensacion = round(compensacion, 2)
            finiquito.total_neto = round(total, 2)

            finiquito.save()
            return redirect('finiquito_lista')
    else:
        form = FiniquitoForm()

    return render(request, 'usuarios/registrar_finiquito.html', {'form': form})

@login_required
def finiquito_nuevo(request):
    if request.method == 'POST':
        form = FiniquitoForm(request.POST)
        if form.is_valid():
            finiquito = form.save()
            return redirect('finiquito_lista')  # Asegúrate de tener esta vista creada
    else:
        form = FiniquitoForm()

    return render(request, 'usuarios/finiquito_nuevo.html', {'form': form})

@login_required
def finiquitos_menu(request):
    return render(request, 'usuarios/finiquitos_menu.html')
